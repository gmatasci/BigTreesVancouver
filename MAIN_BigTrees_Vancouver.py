...
## START ---------------------------------------------------------------------
    ...
    print('Started on %s' % time.strftime('%a, %d %b %Y %H:%M:%S', time.localtime()))
    ...
    arcgis_temp_dir = os.path.join(PARAMS['temp_dir'], 'arcgis_temp')
    tile_las_dir = os.path.join(PARAMS['data_dir'], 'las')
    tile_laz_dir = os.path.join(PARAMS['data_dir'], 'laz')
    tile_denoised_dir = os.path.join(PARAMS['temp_dir'], 'tiles_denoised')
    tile_ht_norm_dir = os.path.join(PARAMS['temp_dir'], 'tiles_ht_norm')
    tile_dem_dir = os.path.join(PARAMS['temp_dir'], 'tiles_dem')
    tile_mask_dir = os.path.join(PARAMS['temp_dir'], 'tiles_mask')
    tile_final_dir = os.path.join(PARAMS['temp_dir'], 'tiles_final')
    tile_segmented_dir = os.path.join(PARAMS['temp_dir'], 'tiles_segmented')
...
## RASTER PROCESSING ----------------------------------------------------------------
    ...
    if PARAMS['raster_processing']:
        ...
        for i_tile, _ in enumerate(dsm_paths):
            ...
            print('tile %d / %d ...' % (i_tile+1, len(dsm_paths)))
            ...
            common_extent.XMin = max(dsm.extent.XMin, mask.extent.XMin)
            common_extent.YMin = max(dsm.extent.YMin, mask.extent.YMin)
            common_extent.XMax = min(dsm.extent.XMax, mask.extent.XMax)
            common_extent.YMax = min(dsm.extent.YMax, mask.extent.YMax)
...
#### LOAD LAYERS INTO MXD PROJECT -------------------------------------------------------------------
    ...
    # if os.path.exists(arcgis_temp_dir):
    #     shutil.rmtree(arcgis_temp_dir)
...
# lastile(in_dir=tile_classif_dir, in_ext='laz', out_dir=tile_final_dir,
